...
main.post('/signup', function(req, res) {
    ...
    if (req.body.username !== "" && req.body.password !== "") {
        ...
        var rand = uuid.v4();
        db.run("INSERT INTO `users` VALUES (?, ?, ?)", req.body.username, "Noob user n°"+rand, req.body.password, function(err) {
            if (err !== null) {
                res.send({err:2, message:'Username already exists'});
            } else {
                var token = uuid.v4();
                res.send({err:0, message:"Noob user n°"+rand, token: token});
                db.run("INSERT INTO `tokens` VALUES (?, ?, ?, ?)", this.lastID, token, parseInt(Date.now() / 1000, 10), "Noob user n°"+rand);
            }
        });
...
main.post('/signin', function(req, res) {
    ...
    db.all("SELECT * FROM users WHERE id=(?) AND password=(?)", req.body.username, req.body.password, function(err, rows) {
        ...
        db.all("SELECT * FROM `tokens` WHERE token=?", token, function(err, rows) {
            if (rows.length !== 0) {
                ...
                socket.on('changeName', function(name) {
                    var db = new sqlite3.Database(dbFile);
                    db.run("UPDATE `users` SET username=? WHERE id=?", name, socket.idUser, function (err) {
                        if (err !== null) {
                            console.log(err);
                            ...
                            console.log("'" + socket.idUser + "' changed username to '" + name +"'");
